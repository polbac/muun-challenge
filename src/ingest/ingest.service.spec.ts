...
describe('IngestService', () => {
    ...
    it('should ingest IPs successfully', async () => {
        ...
        // Temp table creation
        expect(mockQueryRunner.query).toHaveBeenCalledWith(expect.stringContaining('CREATE TABLE ips_temp'));

        // Insertion
        expect(mockQueryRunner.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO ips_temp'), ips);

        // Table swap
        expect(mockQueryRunner.startTransaction).toHaveBeenCalled();
        expect(mockQueryRunner.query).toHaveBeenCalledWith(expect.stringContaining('DROP TABLE ips'));
        expect(mockQueryRunner.query).toHaveBeenCalledWith(expect.stringContaining('RENAME TO ips'));
        ...
        // Cleanup
        expect(mockQueryRunner.query).toHaveBeenCalledWith(expect.stringContaining('DROP TABLE IF EXISTS ips_temp'));
    ...
    it('should handle empty IP list', async () => {
        ...
        expect(mockQueryRunner.query).not.toHaveBeenCalledWith(expect.stringContaining('INSERT INTO ips_temp'), expect.anything());
    });
    ...
    it('should handle large batches', async () => {
        ...
        // 2 calls to INSERT
        const insertCalls = mockQueryRunner.query.mock.calls.filter(call => call[0].includes('INSERT INTO ips_temp'));
