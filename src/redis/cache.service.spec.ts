...
describe('CacheService', () => {
    ...
    describe('setBlockedIp', () => {
        it('should set 1 for blocked', async () => {
            ...
            expect(mockRedis.set).toHaveBeenCalledWith('blocked:1.2.3.4', '1', 'EX', expect.any(Number));
        });

        it('should set 0 for not blocked', async () => {
            await service.setBlockedIp('1.2.3.4', false);
            expect(mockRedis.set).toHaveBeenCalledWith('blocked:1.2.3.4', '0', 'EX', expect.any(Number));
        });
